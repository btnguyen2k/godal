...
func initDataMssql(sqlC *prom.SqlConnect, table string) {
	...
	types := []string{"NVARCHAR(16)", "NVARCHAR(255)", "NCHAR(1)", "BIGINT", "FLOAT(53)", "NVARCHAR(256)",
...
func demoMssqlInsertRows(loc *time.Location, table string, txMode bool) {
	sqlC := createSqlConnectForMssql()
	defer sqlC.Close()
	initDataMssql(sqlC, table)
...
func main() {
	...
	demoMssqlUpsertRows(loc, table, false, "log", "loggging")
	demoMssqlSelectSortingAndLimit(loc, table)
}
